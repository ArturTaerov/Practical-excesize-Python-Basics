...
while (tm0 <= 0) or (tm0 > 86400):
    ...
    if ch < 1:
        ch = 0
    minim = tm0 // 60 - ch * 60  # вычисляю кол-во часов
    if minim < 1:
        minim = 0
    ch = str(ch)  # кол-во часов перевожу в строковый тип
    minim = str(minim)  # кол-во минут перевожу в строковый тип
    sec = str(tm0 % 60)  # кол-во секунд перевожу в строковый тип
    tm1 = [ch, minim, sec]  # создаю список из значений часов, минут и секунд
    for t in range(len(tm1)):
        if len(tm1[t]) != 2:  # если символ один, то добавляю в начало строки "0"
            tm1[t] = "0" + tm1[t]
        if t < len \
                (tm1) - 1:  # вывожу на печать строку с временем в требуемом формате (с разделителем ":" между значениями)
...
maxim = -1
while maxim <= 0:
    number = input("Введите целое положительное целое число, состоящее из более чем 2-х цифр: ")
    for i in range(len(number)):
        if (len(str(number)) > 1) and (str(number[i]) in numbers) :
            if int(number[i]) > maxim:
                maxim = int(number[i])
        else:
            ...
            break
if maxim != -1:
    print ("Самая большая цифра в веденном числе - %d" % maxim)
...
while x:
    ...
    for i in range(len(str(vr)) - 1):
        if (vr
            [i] not in numbers): # or (str(vr[0])) == str(numbers[10]) or ((str(vr[0]) == str(numbers[0])) and ((str(vr[1]) == str(numbers[0])) or (index > 1):
    ...
    for i in range(len(str(rs)) - 1):
        if (rs
            [i] not in numbers): # or (str(rs[0])) == str(numbers[10]) or ((str(rs[0]) == str(numbers[0])) and ((str(rs[1]) == str(numbers[0])) or (index > 1):
            print("Введено неверное значение!")
            y = False
            break
...
while x:
    ...
    if (y != False) and (float(vr ) >0) and (float(rs ) >0):
        ...
        if result > 0:
            ...
            print("Прибыль фирмы в расчете на одного сотрудника составила {:.2f}".format(resul t /int(str)))
            break
        else:
            result = -1 * result
...
while True:
    ...
    while rs < float(resultgoal):
        index += 1
        rs = float(result1) * 1.1 ** (inde x -1)
        print(f"{index}-й день: ", end="")
        print("{:.2f}".format(float(rs)))

    print(f"Ответ: на {index}-й день спортсмен достиг результата — не менее {int(r s/ /1)} км.")
    break
